...
export class SVGViewer implements ComponentFramework.StandardControl<IInputs, IOutputs> {
	...
	//data callout goes here
	private _calloutValue: string;
	...
	public updateView(context: ComponentFramework.Context<IInputs>): void
	{
		...
		this._calloutValue = context.parameters.calloutID.raw!;
		...
		for (let i = 0; i < svgObjCollection.length; i++) {
			...
			if(svgObj != null){
				...
				if(svgObj.getAttribute("data-callout") == this._calloutValue) {
					if(this._useFill) {
   ...
   */
	public onElementClick(event: Event): void {
		...
		let elementRecordcallout = (event.currentTarget as HTMLDivElement).getAttribute("data-callout");			


		if(elementRecordId != null && elementRecordcallout != null ) {
			if(this._useFill) {
			elementPath.style.fill = this.fillSelected;
			}
			this._value = elementRecordId;
			this._calloutValue = elementRecordcallout;
